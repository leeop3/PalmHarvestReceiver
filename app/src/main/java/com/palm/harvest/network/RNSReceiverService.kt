...
class RNSReceiverService : Service() {
    ...
    private fun startBridge(device: BluetoothDevice) {
        // Cancel any existing bridge attempt immediately
        bridgeJob?.cancel()
        
        bridgeJob = serviceScope.launch(Dispatchers.IO) {
            try {
                onStatusUpdate("Resetting Bridge...")
                cleanupResources()
                delay(1000) // Give OS time to release port 7633
                ...
                tcpServer?.reuseAddress = true // Crucial for rapid reconnects
                tcpServer?.bind(InetSocketAddress("127.0.0.1", 7633))
                
                onStatusUpdate("Bridge Ready")
                ...
                // Bidirectional Transparent Pipe
                val p1 = launch {
                ...
                Log.e("PalmHarvest", "Bridge Fail", e)
                cleanupResources()
            }
        }
    }
